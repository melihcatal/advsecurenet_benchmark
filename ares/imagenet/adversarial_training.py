...
def main(args, args_text):
    ...
    trainset = torchvision.datasets.ImageNet(root="/local/scratch/datasets/ImageNet/ILSVRC2012", split='train', transform=transform)
    testset =torchvision.datasets.ImageNet(root="/local/scratch/datasets/ImageNet/ILSVRC2012", split='val', transform=transform)
    ...
    loader_train = torch.utils.data.DataLoader(trainset, batch_size=16, shuffle=False, num_workers=4, sampler = torch.utils.data.distributed.DistributedSampler(trainset, num_replicas=args.world_size, rank=args.rank), drop_last=True)
    loader_eval = torch.utils.data.DataLoader(testset, batch_size=16, shuffle=False, num_workers=4, sampler = torch.utils.data.distributed.DistributedSampler(testset, num_replicas=args.world_size, rank=args.rank), drop_last=True)
    # setup loss function
...
# Function to ensure correct input and target shapes
def preprocess_input_target(input, target):
    # Ensure input shape is (batch_size, 3, 32, 32)
    if input.shape[1] != 3:
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        input, target = preprocess_input_target(input, target)
        if mixup_fn is not None:
