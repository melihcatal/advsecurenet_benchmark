...
def main(args, args_text):
    ...
    loader_train = torch.utils.data.DataLoader(trainset, batch_size=128, shuffle=False, num_workers=2, sampler = torch.utils.data.distributed.DistributedSampler(trainset, num_replicas=args.world_size, rank=args.rank), drop_last=True)
    loader_eval = torch.utils.data.DataLoader(testset, batch_size=100, shuffle=False, num_workers=2, sampler = torch.utils.data.distributed.DistributedSampler(testset, num_replicas=args.world_size, rank=args.rank), drop_last=True)
    # setup loss function
